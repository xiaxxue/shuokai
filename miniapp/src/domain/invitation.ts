...
export function parseInvitationContext(value: unknown): InvitationContext {
  ...
  const title = boundedText(value.title, 40) || "关于这次沟通";
  const summary = boundedText(value.summary, 300) || (topic
    ? `发起方确认的背景是：${topic}`
    : "发起方想邀请你先了解一件具体发生的事，再从你的视角讲讲当时的情况。");
  if (!inviterName) throw new Error("邀请说明格式无效，请稍后重试。");
  return { inviterName, topic, title, summary, confirmedSummary: value.confirmedSummary === true };
}
...
export function invitationContextFromEditableExpression(
  ...
  const title = boundedText(expression.invitation.title, 40) || "关于这次沟通";
  const summary = boundedText(expression.invitation.summary, 300);
  return {
    inviterName,
    topic,
    title,
    ...
    confirmedSummary: Boolean(summary),
  };
}
