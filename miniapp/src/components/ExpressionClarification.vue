        ...
        <view class="expression-card" aria-label="AI 协助整理的可编辑表达卡">
          <view class="card-heading">
            <view class="card-heading-copy">
              ...
              <text class="card-title">当前表达卡</text>
              <text class="card-caption">{{ modeTitle }} · 还没有分享给对方</text>
            </view>
...
function commitBufferedEdits() {
  ...
  if (invitationBuffer.value.title !== props.modelValue.invitation.title) {
    emit("update-invitation", "title", invitationBuffer.value.title);
  }
  if (invitationBuffer.value.summary !== props.modelValue.invitation.summary) {
    emit("update-invitation", "summary", invitationBuffer.value.summary);
  }
}
...
.selected-path button::after, .direct-edit-trigger::after { border: 0; }
